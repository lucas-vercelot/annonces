//
//  ClassifiedAdFiltererTests.swift
//  AnnoncesTests
//
...
class ClassifiedAdFiltererTests: XCTestCase {

    func test__given_several_unordered_classifiedAds_and_no_category_filer__filterer_should_return_all_ordered_items() {
        // Given
        let unfilteredClassifiedAds = getUnfilteredEnrichedClassifiedAds()
        let expectedFilteredClassifiedAds = getFilteredEnrichedClassifiedAds()
        
        // When
        let filterer = ClassifiedAdFilterer()
        let filteredClassifiedAds = filterer.filter(enrichedClassifiedAds: unfilteredClassifiedAds, for: AdCategory(id: 4, name: "Maison"))
        
        // Then
        XCTAssertFalse(filteredClassifiedAds.isEmpty)
        XCTAssertEqual(filteredClassifiedAds, expectedFilteredClassifiedAds)
    }
}

// MARK: - EnrichedClassifiedAds data

extension ClassifiedAdFiltererTests {
 
    func getUnfilteredEnrichedClassifiedAds() -> [EnrichedClassifiedAd] {
        return [
    ...
    func getFilteredEnrichedClassifiedAds() -> [EnrichedClassifiedAd] {
        return [
            EnrichedClassifiedAd(
                from:
                    ClassifiedAd(
