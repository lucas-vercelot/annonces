//
//  ClassifiedAdOrdererTests.swift
//  AnnoncesTests
//
...
class ClassifiedAdOrdererTests: XCTestCase {

    func test__given_several_unordered_classifiedAds_and_no_category_filer__filterer_should_return_all_ordered_items() {
        // Given
        let unorderedClassifiedAds = getUnorderedEnrichedClassifiedAds()
        let expectedOrderedClassifiedAds = getOrderedEnrichedClassifiedAds()
        
        // When
        let orderer = ClassifiedAdOrderer()
        let orderedEnrichedClassifiedAds = orderer.orderByDateAndUrgent(enrichedClassifiedAds: unorderedClassifiedAds)
        
        // Then
        XCTAssertFalse(orderedEnrichedClassifiedAds.isEmpty)
        XCTAssertEqual(orderedEnrichedClassifiedAds, expectedOrderedClassifiedAds)
    }
}

// MARK: - EnrichedClassifiedAds data

extension ClassifiedAdOrdererTests {
 
    func getUnorderedEnrichedClassifiedAds() -> [EnrichedClassifiedAd] {
        return [
    ...
    func getOrderedEnrichedClassifiedAds() -> [EnrichedClassifiedAd] {
        return [
            EnrichedClassifiedAd(
                from:
                    ClassifiedAd(
